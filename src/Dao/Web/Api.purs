...
createConfig ::
  Fn2 Ctl.ContractEnv CreateConfigParams (Promise CreateConfigResult)
...
createIndexConfig = mkContractCall1 createIndexConfig'
  ...
  createIndexConfig' = do
    indexTokenName <-
      Ctl.liftContractM "Could not make index token name" $ Utils.mkTokenName
        "index1"
    ...
    voteFungibleTokenName <-
      Ctl.liftContractM "Could not make voteNft token name" $ Utils.mkTokenName
        "vote_fungible"
...
upgradeConfig ::
  Fn2 Ctl.ContractEnv UpgradeConfigParams (Promise TransactionHash)
upgradeConfig = mkContractCall2 Dao.upgradeConfig

createTreasuryFund ::
  Fn2 Ctl.ContractEnv CreateTreasuryFundParams (Promise ContractResult)
createTreasuryFund = mkContractCall2 Dao.createTreasuryFund

deployReferenceScriptsOne ::
  Fn2 Ctl.ContractEnv ValidatorParams (Promise TransactionHash)
deployReferenceScriptsOne =
  mkContractCall2
    \(ScriptArguments.ValidatorParams { vpConfigSymbol, vpConfigTokenName }) ->
      Dao.deployReferenceScriptsOne
        (Component.mkValidatorConfig vpConfigSymbol vpConfigTokenName)
...
deployReferenceScriptsThree =
  mkContractCall2
    \(ScriptArguments.ValidatorParams { vpConfigSymbol, vpConfigTokenName }) ->
      Dao.deployReferenceScriptsThree
        (Component.mkValidatorConfig vpConfigSymbol vpConfigTokenName)

deployAllReferenceScripts ::
  Fn2 Ctl.ContractEnv ValidatorParams (Promise (Array TransactionHash))
deployAllReferenceScripts =
  mkContractCall2
    \(ScriptArguments.ValidatorParams { vpConfigSymbol, vpConfigTokenName }) ->
      do
        let
          validatorConfig = Component.mkValidatorConfig vpConfigSymbol
            vpConfigTokenName
...
createFungible ::
  Fn2 Ctl.ContractEnv CreateFungibleParams (Promise ContractResult)
createFungible = mkContractCall2 Dao.createFungible

createProposal ::
  Fn2 Ctl.ContractEnv CreateProposalParams (Promise ContractResult)
createProposal = mkContractCall2 Dao.createProposal

voteOnProposal ::
  Fn2 Ctl.ContractEnv VoteOnProposalParams (Promise VoteOnProposalResult)
...
getAllProposals ::
  Fn2 Ctl.ContractEnv QueryProposalParams (Promise (Array QueryResult))
...
getProposalByTokenName ::
  Fn3 Ctl.ContractEnv QueryProposalParams TokenName
    (Promise (JsMaybe QueryResult))
