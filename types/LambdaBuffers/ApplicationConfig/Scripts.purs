module LambdaBuffers.ApplicationConfig.Scripts (Script(..), Scripts(..), Bytes(..)) where
...
instance Prelude.Eq Script where
  eq = (\x0 -> (\x1 -> let Script x2 = x0 in let Script x3 = x1 in Prelude.(==) (x2) (x3) ) )

instance LambdaBuffers.Runtime.Prelude.Json Script where
  toJson = (\x0 -> let Script x1 = x0 in LambdaBuffers.Runtime.Prelude.toJson (x1) )
  fromJson = (\x0 -> Prelude.(>>=) (LambdaBuffers.Runtime.Prelude.fromJson (x0)) ((\x1 -> Data.Either.Right (Script x1) )) )

instance Prelude.Eq Scripts where
  eq = (\x0 -> (\x1 -> Prelude.(==) ((Data.Newtype.unwrap x0).configPolicy) ((Data.Newtype.unwrap x1).configPolicy) ) )

instance LambdaBuffers.Runtime.Prelude.Json Scripts where
  toJson = (\x0 -> LambdaBuffers.Runtime.Prelude.jsonObject ([(Data.Tuple.Tuple ("configPolicy") (LambdaBuffers.Runtime.Prelude.toJson ((Data.Newtype.unwrap x0).configPolicy)))]) )
  fromJson = LambdaBuffers.Runtime.Prelude.caseJsonObject ((\x0 -> LambdaBuffers.Runtime.Prelude.jsonField ("configPolicy") (x0) ((\x1 -> Prelude.(>>=) (LambdaBuffers.Runtime.Prelude.fromJson (x1)) ((\x2 -> Data.Either.Right (Scripts { configPolicy : x2 }) )) )) ))
...
instance eqBytes :: Prelude.Eq Bytes where
  eq l r = unsafePerformEffect $ ArrayBuffer.eq (Data.Newtype.unwrap l) (Data.Newtype.unwrap r)
